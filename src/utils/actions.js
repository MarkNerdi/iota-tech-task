...
// decimal
export function decimal(node, amount = 2) {
    const format = () => (node.value = +parseFloat(node.value).toFixed(amount));
    node.addEventListener("blur", format);
    return {
        destroy() {
            node.removeEventListener("blur", format);
        }
    };
}
